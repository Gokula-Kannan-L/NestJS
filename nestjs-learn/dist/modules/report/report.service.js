...
let ReportService = exports.ReportService = class ReportService {
    getAllReports(type) {
        return data_1.data.report.filter(i => i.type === type).map(report => new report_dto_1.ReportResponseDto(report));
    }
    getReportById(type, id) {
        const report = data_1.data.report.filter(i => i.type === type).find(j => j.id === id);
        if (!report)
            return;
    ...
    updateReport(type, id, body) {
        const reportToUpdate = data_1.data.report.filter(i => i.type === type).find(j => j.id === id);
        if (!reportToUpdate)
            return;
        const reportIndex = data_1.data.report.findIndex(k => k.id === reportToUpdate.id);
    ...
    deleteReport(id) {
        const reportIndex = data_1.data.report.findIndex(i => i.id === id);
        if (reportIndex === -1)
            return;
